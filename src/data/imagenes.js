// Fotografias de Wikimedia Commons (dominio publico o licencias libres).
// Se centralizan aqui porque varias secciones reutilizan la misma imagen.
const C = 'https://upload.wikimedia.org/wikipedia/commons'

export const foto = {
  // Panoramicas y simbolos
  panoramica: `${C}/d/de/View_of_Monterrey_%282015%29.jpg`,
  cerroSilla: `${C}/f/f7/CerroDeLaSilla201806p1.jpg`,
  cerroSillaVertical: `${C}/6/61/Cerro_de_la_Silla_Mty.jpg`,
  cerroSillaJuarez: `${C}/1/14/Vista_del_Cerro_de_la_Silla._2.jpg`,
  mitras: `${C}/2/20/Cerro-mitras-desde-nido-aguiluchos.jpg`,
  topoChico: `${C}/a/ad/Topo_chico_desde_mitras.jpg`,

  // Centro de Monterrey
  macroplaza: `${C}/0/03/Macroplaza_de_Monterrey_y_alrededores_-_6.jpg`,
  faroComercio: `${C}/a/a9/Faro_del_Comercio_en_Monterrey.jpg`,
  catedral: `${C}/0/05/Monterrey_-_Catedral_de_la_Inmaculada_Concepci%C3%B3n_-_4.jpg`,
  marco: `${C}/8/80/Fachada_MARCO.jpg`,
  museoHistoria: `${C}/9/99/Museo_Historia_Mexicana_Monterrey.jpg`,
  barrioAntiguo: `${C}/6/69/Barrio_Antiguo_de_Monterrey_en_2017.jpg`,
  barrioAntiguoNoche: `${C}/4/4d/Saturday_night_in_Barrio_Antiguo_Monterrey.jpg`,
  centro: `${C}/4/4b/Centro_de_Monterrey_07032021.jpg`,

  // Fundidora y Santa Lucia
  fundidora: `${C}/0/07/Parque_Fundidora%2C_Monterrey1.jpg`,
  horno3: `${C}/7/7b/Museo_del_acero.jpg`,
  santaLucia: `${C}/e/e8/Paseo_Santa_Luc%C3%ADa_-_Santa_Luc%C3%ADa_Riverwalk.jpg`,

  // Sierra y naturaleza
  huasteca: `${C}/e/e7/Huasteca_Canyon%2C_Nuevo_Leon._%285663887022%29.jpg`,
  colaDeCaballo: `${C}/f/f8/Cola_de_Caballo.JPG`,
  grutasGarcia: `${C}/0/0c/Grutas_de_Garc%C3%ADa.jpg`,
  presaLaBoca: `${C}/1/17/Presa_La_Boca%2C_Santiago%2C_Nuevo_Le%C3%B3n%2C_M%C3%A9xico_-_3.jpg`,
  chipinque: `${C}/7/72/Chipinque%2C_San_Pedro%2C_M%C3%A9xico.jpg`,
  laPastora: `${C}/b/b7/Rio_la_Silla_en_el_Bosque_la_Pastora.png`,

  // Historia e industria
  montemayorMural: `${C}/f/fb/Diego_de_Montemayor_Mural.jpg`,
  montemayor: `${C}/8/80/Diego_de_Montemayor.jpg`,
  cerveceria1890: `${C}/6/6c/Cervecer%C3%ADa_Cuauhtemoc_Monterrey_1890.jpg`,
  tecDeMonterrey: `${C}/b/bf/Hub_de_Innovaci%C3%B3n_y_Emprendimiento_-_Campus_Monterrey_%28ITESM%29.jpg`,
  metrorrey: `${C}/e/e6/CRRC_MM-24_Metrorrey.jpg`,
  puenteUnidad: `${C}/a/ac/Puente_Atirantado.jpg`,
  estadioBBVA: `${C}/e/e1/Estadio_BBVA_Bancomer.jpg`,
  aeropuerto: `${C}/0/0d/Aeropuerto_Internacional_de_Monterrey.jpg`,

  // Personas
  alfonsoReyes: `${C}/5/5c/Alfonso_Reyes_Ochoa_%28cropped%29.jpg`,
  frayServando: `${C}/5/58/Frayservando.jpg`,
  garzaSada: `${C}/c/cb/Eugenio_Garza_Sada_%28cropped%29.jpg`,
  celsoPina: `${C}/b/b1/MX_TV_A%C3%91O_DUAL_MEXICO-ALEMANIA_CELSO_PI%C3%91A_%2835052491012%29.jpg`,
  gloriaTrevi: `${C}/3/30/Gloria_Trevi_Latin_Grammys_2021.jpg`,
  granSilencio: `${C}/4/4c/El_Gran_Silencio_2023.jpg`,

  // Municipios
  sanPedro: `${C}/6/64/DelValleCity.jpg`,
  sanNicolas: `${C}/6/65/Avenida_Universidad_-_San_Nicol%C3%A1s_de_los_Garza.JPG`,
  apodaca: `${C}/a/a7/SAN_FRANCISCO_DE_ASIS.jpg`,
  escobedo: `${C}/2/24/EscobedoNL.jpg`,
  santaCatarina: `${C}/5/53/Panor%C3%A1mica_de_Santa_Catarina.jpg`,
  garcia: `${C}/d/d7/Capilla_de_San_El%C3%ADas.jpg`,
  santiago: `${C}/d/d8/Santiago_Ap%C3%B3stol_desde_el_Mirador.jpg`,
  cadereyta: `${C}/e/ed/Plaza_principal_de_Cadereyta_Jim%C3%A9nez_con_estatua_de_Miguel_Hidalgo.jpg`,
  salinasVictoria: `${C}/b/bc/Templo_de_Nuestra_Se%C3%B1ora_de_Guadalupe_en_Salinas_Victoria.jpg`,
  pesqueria: `${C}/8/8e/Panoramic_view_of_town_hall_Pesquer%C3%ADa.jpg`,
  elCarmen: `${C}/a/a4/Palacio_Municipal_de_El_Carmen.jpg`,
  // De General Zuazua y Cienega de Flores no hay foto libre en Commons: sus
  // fichas se quedan con el marcador de posicion.

  // Cocina regia
  cabrito: `${C}/b/b9/Cabrito_-_Monterrey.JPG`,
  cabritos: `${C}/a/af/Cabritos_in_Monterrey.jpg`,
  machacado: `${C}/1/1a/Machacado_con_huevo.jpg`,
  machaca: `${C}/9/96/Huevo_con_machaca.jpg`,
  asadoPuerco: `${C}/7/7d/PAGA_asadodepuerco_02.jpg`,
  discada: `${C}/f/f1/Discada_1.jpg`,
  frijolesCharros: `${C}/3/34/Frijoles_charros_10293.jpg`,
  carneAsada: `${C}/7/70/Mexican_BBQ_gas_tank_grill.jpg`,
  glorias: `${C}/a/a4/Bolsa_de_Glorias_de_Linares.jpg`,
  mercadoJuarez: `${C}/9/95/Mercado_Ju%C3%A1rez_de_Monterrey_-_2.jpg`,
  mercadoJuarez2: `${C}/2/24/Mercado_Ju%C3%A1rez_de_Monterrey_-_3.jpg`,
  carneAsadaPlato: `${C}/d/d0/Carne_asada_%284472586086%29.jpg`,
  // Vertical, para el escaparate del platillo insignia
  carneAsadaVertical: `${C}/6/6a/Ima20180329_153312.jpg`,
  parrillada: `${C}/8/85/Parrillada_Carne_asada.jpg`,
  // La tortilla de harina es de todo el norte; estas fotos son de Sonora.
  tortillasHarina: `${C}/c/cd/Se%C3%B1ora_haciendo_tortillas_de_harina_01.jpg`,
  tortillasHarina2: `${C}/f/f3/Tortillas_de_harina_en_proceso_01.jpg`,

  // Bustamante: Pueblo Magico a 107 km al noroeste
  bustamante: `${C}/d/dd/Bustamante_-_panoramio_-_Tereso_Hern%C3%A1ndez.jpg`,
  grutasBustamante: `${C}/7/7b/Grutas_de_Bustamante_1.jpg`,
}
