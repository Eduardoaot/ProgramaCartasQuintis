...
export default function Antecedentes() {
  return (
    ...
    >
      ...
      <Reveal
        variant="left"
        className="mx-auto max-w-3xl rounded-2xl border-l-4 border-rosa bg-(--surface) p-6 text-left shadow-[0_6px_16px_rgba(142,36,83,0.10)]"
      >
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {firmadas.sets.map((set, i) => (
            <Reveal
              ...
              delay={(i % 3) * 80}
              className="flex flex-col gap-3 rounded-2xl border-2 border-(--hairline) bg-(--surface) p-4 shadow-[0_6px_16px_rgba(142,36,83,0.10)] transition duration-300 hover:-translate-y-1 hover:border-rosa"
            >
              <a
                href={set.enlace}
                target="_blank"
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-(--page-text)/70">✒️ Cartas SP firmadas · {set.idiomas}</p>
              </div>
            </Reveal>
          ))}
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {valor.cartas.map((c, i) => (
            <Reveal
              ...
              variant="scale"
              delay={i * 90}
              className="group flex flex-col gap-3 rounded-2xl border-2 border-(--hairline) bg-(--surface) p-4 text-center shadow-[0_6px_16px_rgba(142,36,83,0.10)] transition duration-300 hover:-translate-y-2 hover:border-rosa hover:shadow-[0_20px_35px_rgba(142,36,83,0.28)]"
            >
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="mt-auto rounded-lg bg-linear-to-br from-vino to-rosa px-2 py-1.5 text-sm font-extrabold text-white transition hover:brightness-110"
              >
                {c.valor}
              </a>
            </Reveal>
