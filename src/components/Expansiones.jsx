...
function SetCard({ set, delay, side }) {
  return (
    <Reveal
      variant={side}
      delay={delay}
      className="group flex flex-col gap-3 overflow-hidden rounded-2xl border-2 border-(--hairline) bg-(--surface) p-5 shadow-[0_6px_16px_rgba(142,36,83,0.10)] transition duration-300 hover:-translate-y-2 hover:border-rosa hover:shadow-[0_20px_35px_rgba(142,36,83,0.28)]"
    >
      <BoxPhoto set={set} />

      <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
        <span className="inline-flex items-center justify-center rounded-lg bg-linear-to-br from-vino to-rosa px-2.5 py-1 text-sm font-extrabold text-white">
          {set.codigo}
        </span>
      ...
      <dl className="mt-auto flex flex-col gap-1 pt-1 text-sm">
        {set.totalCartas && (
          <div>
            ...
            <dd className="inline text-(--page-text)/80">{set.totalCartas}</dd>
          </div>
        )}
        {set.destacado && (
          <div>
    ...
    >
      {fichaComun && (
        <Reveal variant="up" className="mx-auto mb-10 flex max-w-lg flex-wrap justify-center gap-3 text-sm">
          <span className="rounded-full bg-(--surface) px-4 py-1.5 font-semibold text-vino ring-1 ring-(--hairline) dark:text-rosa">
