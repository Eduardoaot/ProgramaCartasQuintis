...
export default function Card({ title, subtitle, images = [], wide = false, delay = 0, children }) {
  return (
    <Reveal
      delay={delay}
      className={`group relative flex flex-col gap-2 overflow-hidden rounded-2xl border-2 border-[var(--hairline)] bg-[var(--surface)] p-6 shadow-[0_6px_16px_rgba(142,36,83,0.10)] transition duration-300 hover:-translate-y-2 hover:border-rosa hover:shadow-[0_20px_35px_rgba(142,36,83,0.28)] ${
        wide ? 'sm:col-span-2' : ''
      }`}
    >
      {/* destello */}
