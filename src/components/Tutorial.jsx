...
function Objetivo() {
  ...
  return (
    <Reveal
      variant="zoom"
      className="mx-auto max-w-3xl overflow-hidden rounded-3xl bg-linear-to-br from-vino via-rosa to-vino p-8 text-center text-white shadow-xl"
    >
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {mazos.items.map((m, i) => (
          <Reveal
            ...
            variant="up"
            delay={i * 90}
            className="group relative rounded-2xl border-2 border-(--hairline) bg-(--surface) p-6 text-left shadow-[0_6px_16px_rgba(142,36,83,0.10)] transition duration-300 hover:-translate-y-2 hover:border-rosa"
          >
            {/* pila de cartas */}
            <div className="mb-4 flex h-16 items-end gap-1.5">
              {[0, 1, 2, 3].map((k) => (
                <span
                  key={k}
                  className="w-6 rounded-md bg-linear-to-b from-rosa to-vino transition-all duration-300 group-hover:translate-y-[-3px]"
    ...
    <div className="mt-16 space-y-6">
      ...
      {tiposCarta.map((t, i) => (
        <Reveal
          ...
          className={`mx-auto flex max-w-3xl flex-col items-center gap-5 rounded-2xl border-2 border-(--hairline) bg-(--surface) p-5 shadow-[0_6px_16px_rgba(142,36,83,0.10)] transition duration-300 hover:border-rosa sm:gap-7 ${
        ...
        >
          ...
          <div className="text-left">
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-md bg-linear-to-br from-vino to-rosa px-2 py-0.5 text-xs font-extrabold text-white">
                {t.codigo}
              </span>
  ...
  return (
    <div className="mt-16">
      ...
      <Reveal variant="scale" className="mx-auto max-w-3xl">
        <figure className="overflow-hidden rounded-2xl border-2 border-(--hairline) bg-(--surface-sunken) p-2 shadow-[0_12px_34px_rgba(142,36,83,0.20)]">
          <img
            src={arena.playmat}
      ...
      <div className="mx-auto grid max-w-4xl gap-4 md:grid-cols-4">
        {turno.fases.map((f, i) => (
          <Reveal
            ...
            variant="up"
            delay={i * 110}
            className="relative rounded-2xl border-2 border-(--hairline) bg-(--surface) p-5 text-left shadow-[0_6px_16px_rgba(142,36,83,0.10)] transition duration-300 hover:-translate-y-2 hover:border-rosa"
          >
            <span className="absolute -top-3 left-5 flex h-7 w-7 items-center justify-center rounded-full bg-linear-to-br from-vino to-rosa text-sm font-bold text-white ring-4 ring-(--page-bg)">
              {i + 1}
            </span>
      ...
      <ol className="relative mx-auto max-w-2xl border-l-2 border-rosa/40 pl-8">
        {aproximacion.pasos.map((p, i) => (
          <Reveal key={p.titulo} as="li" variant="left" delay={i * 100} className="relative mb-6 last:mb-0">
            <span className="absolute -left-10.5 flex h-8 w-8 items-center justify-center rounded-full bg-linear-to-br from-vino to-rosa text-sm font-bold text-white ring-4 ring-(--page-bg)">
              {i + 1}
            </span>
    ...
    <div className="flex flex-col items-center">
      <span
        className={`flex h-14 w-14 items-center justify-center rounded-2xl text-2xl font-extrabold text-white ${
          tone === 'vino' ? 'bg-vino' : 'bg-linear-to-br from-rosa to-vino'
        }`}
      >
        {value}
      </span>
...
function ComoGanar() {
  return (
    <div className="mt-16">
      ...
      <Reveal
        variant="zoom"
        className="mx-auto max-w-3xl rounded-2xl bg-linear-to-br from-vino to-rosa p-8 text-center text-white shadow-xl"
      >
        ...
        {keywords.map((kw, i) => (
          <Reveal key={kw.k} variant="scale" delay={i * 55} className="flip h-28" tabIndex={0}>
            <div className="flip-inner">
              ...
              <div className="flip-face flip-back rounded-xl bg-linear-to-br from-vino to-rosa p-4 text-center text-white">
