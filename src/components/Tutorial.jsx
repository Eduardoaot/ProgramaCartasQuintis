      ...
      <div className="grid gap-6 md:grid-cols-2">
        {tutorial.tiposCarta.map((t, i) => (
          <Reveal
            key={t.nombre}
            delay={i * 70}
            className="rounded-2xl border-2 border-rosa/25 bg-white p-6 text-left shadow-[0_6px_16px_rgba(142,36,83,0.08)] transition duration-300 hover:-translate-y-1 hover:border-rosa hover:shadow-xl"
          >
            <h3 className="mb-2 text-lg font-bold text-rosa">{t.nombre}</h3>
            <p className="text-sm leading-relaxed text-tinta/90">{t.texto}</p>
          </Reveal>
        ))}
      </div>

      {/* Reglas generales */}
      <Reveal className="mx-auto mt-12 max-w-3xl rounded-2xl bg-rosa-claro p-8 text-left">
        ...
        <ol className="space-y-3">
          {tutorial.reglas.map((r, i) => (
            <li key={i} className="flex gap-3">
              <span className="flex h-7 w-7 flex-none items-center justify-center rounded-full bg-vino text-sm font-bold text-white">
                {i + 1}
              </span>
              <span className="pt-0.5 text-sm text-tinta/90">{r}</span>
            </li>
          ))}
        </ol>
      </Reveal>
