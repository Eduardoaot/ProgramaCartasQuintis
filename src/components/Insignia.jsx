...
export default function Insignia() {
  ...
  return (
    <section
      ref={ref}
      aria-label="El platillo insignia de Nuevo León"
      className="relative overflow-hidden bg-[#08131b] px-6 py-24 text-white sm:py-28"
    >
      {/* Halo de luz */}
      <div
        aria-hidden="true"
        ...
        style={{
          background:
            'radial-gradient(circle, rgba(223,161,58,0.38), rgba(207,90,34,0.18) 45%, transparent 70%)',
        }}
      />
      ...
      <div className="relative mx-auto flex max-w-5xl flex-col items-center gap-12 lg:flex-row lg:gap-16">
        ...
        >
          <span className="inline-block rounded-lg bg-linear-to-br from-oro to-naranja px-3 py-1 text-sm font-extrabold tracking-[0.3em] text-sierra">
            {s.codigo}
          </span>
          <h3 className="mt-4 text-3xl font-extrabold sm:text-4xl">{s.nombre}</h3>
          <p className="mt-1 font-semibold text-oro">{s.titular}</p>
          ...
          <dl className="mx-auto mt-6 grid max-w-md gap-4 text-left sm:grid-cols-3 lg:mx-0">
            {[
              ...
              <div key={label}>
                <dt className="text-[10px] font-semibold uppercase tracking-widest text-oro/80">
                  {label}
                </dt>
          ...
          <div
            className={`mt-8 inline-flex items-baseline gap-3 rounded-2xl bg-white/10 px-6 py-4 ring-1 ring-oro/40 transition-all duration-700 ${
          ...
          >
            ...
            <span className="text-2xl font-extrabold text-oro sm:text-3xl">{s.valor}</span>
          </div>
